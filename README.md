...
번역 병합 이후에도 일부 리뷰가 누락되거나 깨진 문자열이 존재하여, 별도로 anime_sentiment_1_cleaned.csv를 생성하고 결측 및 오류 데이터를 제거하여 최종 학습에 사용하였다.
...
기준	내용
추출 비율	전체의 약 4%, 3,000건 샘플링 (Positive/Negative 균형 유지)
분포 유지	클래스 균형 (stratify), 비율 유지한 학습/검증 분할 (80:20)

이러한 방식은 전체 데이터의 감정 분포를 반영하면서도 일반화 성능을 확보하는 데 효과적입니다.
특히 rate 컬럼이 불균형한 상태에서 단순 랜덤 샘플링을 피하고 class-wise 균형을 유지한 것이 성능 향상에 기여했습니다.



## MobileBERT Fine-tuning 결과
처음에는 Positive / Neutral / Negative로 구성된 3진 분류를 시도했으나,
Neutral 클래스는 감정 경계가 모호하고 데이터 분포도 불균형하여 정확도가 50% 내외로 낮게 나왔다.

이에 따라 Positive / Negative의 2진 분류로 재구성했고,
그 결과 Validation Accuracy가 85% 이상까지 상승하는 등 현저한 개선을 보였다.

중립 클래스 제거로 모델이 감정을 더 명확히 구분할 수 있게 되었으며,
데이터의 의미적 일관성과 예측 성능 모두에서 이득을 얻었다.
...
왼쪽(긍정 Top5) 그래프와 오른쪽(부정 Top5)그래프는 위에 설명한 애니 순서대로 긍부정 표를 시각화 하여 나타낸 것이다.
